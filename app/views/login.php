        ...
        <div
            ...
            <div id="step-phone" class="block space-y-4">
                <div>
                    ...
                    <div class="relative">
                        <i data-lucide="phone" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500"
                            style="width:18px;height:18px;"></i>
                        <input type="tel" id="auth-phone" name="username" autocomplete="username" inputmode="numeric"
                            onkeydown="if(event.key === 'Enter') handleCheckPhone(event)"
                            class="w-full bg-black/20 border border-white/10 rounded-xl py-3 pr-10 pl-4 text-white text-sm focus:border-brand-red focus:ring-1 focus:ring-brand-red outline-none transition text-left"
                            placeholder="09123456789" dir="ltr">
                    </div>
                </div>
                <button onclick="handleCheckPhone(event)"
                    class="w-full bg-brand-red hover:bg-red-700 text-white font-bold py-3.5 rounded-xl transition text-sm shadow-[0_4px_15px_rgba(225,6,0,0.2)]">
                    مرحله بعد
                </button>
            </div>
            ...
            <div id="step-password" class="hidden space-y-4">
                <div class="flex items-center justify-between bg-black/20 p-3 rounded-xl border border-white/5 mb-4">
                    ...
                    <button onclick="location.reload()" class="text-xs text-gray-400 hover:text-white transition">تغییر
                        شماره</button>
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-400 mb-2">رمز عبور</label>
                    <div class="relative">
                        <i data-lucide="lock" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500"
                            style="width:18px;height:18px;"></i>
                        <input type="password" id="auth-password" name="password" autocomplete="current-password"
                            onkeydown="if(event.key === 'Enter') handleLoginPassword(event)"
                            class="w-full bg-black/20 border border-white/10 rounded-xl py-3 pr-10 pl-10 text-white text-sm focus:border-brand-red focus:ring-1 focus:ring-brand-red outline-none transition text-left"
                ...
                <button onclick="handleLoginPassword(event)"
                    class="w-full bg-brand-red hover:bg-red-700 text-white font-bold py-3.5 rounded-xl transition text-sm shadow-[0_4px_15px_rgba(225,6,0,0.2)]">
                    ورود به حساب
                </button>
                <div class="text-center pt-2">
                    <button onclick="switchToOtpLogin()"
            ...
            <div id="step-otp" class="hidden space-y-4">
                ...
                <div id="new-user-fields" class="hidden space-y-4 mb-4">
                    <div>
                        ...
                        <div class="relative">
                            <i data-lucide="user" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500"
                                style="width:18px;height:18px;"></i>
                            <input type="text" id="auth-fullname"
                                class="w-full bg-black/20 border border-white/10 rounded-xl py-3 pr-10 pl-4 text-white text-sm focus:border-brand-red outline-none transition pr-10"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <i data-lucide="key" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500"
                                style="width:18px;height:18px;"></i>
                            <input type="password" id="auth-new-password" name="new-password"
                                autocomplete="new-password"
                                onkeydown="if(event.key === 'Enter') document.getElementById('auth-otp-code').focus()"
                                class="w-full bg-black/20 border border-white/10 rounded-xl py-3 pr-10 pl-10 text-white text-sm focus:border-brand-red outline-none transition text-left"
                ...
                <div>
                    ...
                    <input type="text" id="auth-otp-code" name="otp" autocomplete="one-time-code" inputmode="numeric"
                        onkeydown="if(event.key === 'Enter') handleVerifyOtp(event)"
                        class="w-full bg-black/20 border border-white/10 rounded-xl py-3 px-4 text-center text-white text-lg tracking-[0.5em] font-mono focus:border-brand-red focus:ring-1 focus:ring-brand-red outline-none transition"
                        placeholder="- - - - -" maxlength="5" dir="ltr">
                </div>

                <button onclick="handleVerifyOtp(event)"
                    class="w-full bg-brand-red hover:bg-red-700 text-white font-bold py-3.5 rounded-xl transition text-sm shadow-[0_4px_15px_rgba(225,6,0,0.2)]">
                    تایید و ادامه
                </button>
                ...
                <div class="flex items-center justify-between text-xs pt-2">
                    ...
                    <button id="resend-otp-btn" onclick="requestOtp()"
