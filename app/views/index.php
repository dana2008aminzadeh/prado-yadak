...
<body class="bg-brand-dark text-white overflow-x-hidden antialiased">
    ...
    <!-- بخش جدیدترین قطعات سایت (با سئوی کامل Microdata و انیمیشن‌های پیشرفته) -->
    <?php if (!empty($newest_parts)): ?>
        <section id="latest-products" class="py-16 md:py-24 bg-brand-dark relative overflow-hidden border-t border-white/5">
            <!-- افکت نوری پس‌زمینه -->
            <div
                class="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_top_left,rgba(225,6,0,0.04),transparent_50%)] pointer-events-none">
            </div>

            <div class="max-w-7xl mx-auto px-4 relative z-10">
                <!-- هدر بخش -->
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-12 gap-5 scroll-reveal">
                    <div>
                        <span class="text-brand-accent text-sm font-bold tracking-widest flex items-center gap-2">
                            <span class="w-2 h-2 rounded-full bg-brand-red animate-pulse"></span>
                            تکمیل انبار
                        </span>
                        <h2 class="text-3xl md:text-4xl font-black mt-3 text-white">جدیدترین قطعات اضافه شده</h2>
                    </div>
                    <a href="/parts"
                        class="border border-white/10 hover:border-brand-red px-5 py-3 rounded-xl text-xs font-bold transition flex items-center gap-2 bg-brand-grey/40 hover:bg-brand-red hover:text-white shrink-0 group shadow-sm">
                        مشاهده کاتالوگ قطعات
                        <i data-lucide="arrow-left"
                            class="w-4 h-4 group-hover:-translate-x-1.5 transition-transform duration-300"></i>
                    </a>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    <?php
                    $delay = 0;
                    foreach ($newest_parts as $p):
                        $safeSlug = urlencode($p['slug']);
                        $img = !empty($p['images']) ? "/image?id=" . e($p['images'][0]) : "/assets/logo/logo.webp";
                        $price = number_format($p['price']);
                        $carModelName = $GLOBALS['car_models'][$p['model']]['name'] ?? $p['model'];
                        ?>
                        <!-- تگ Article با پشتیبانی از Schema.org Product برای سئو -->
                        <article itemscope itemtype="https://schema.org/Product"
                            class="bg-brand-grey border border-white/5 rounded-2xl overflow-hidden group hover:border-brand-red/50 transition-all duration-500 ease-out flex flex-col justify-between relative hover:shadow-[0_20px_40px_-10px_rgba(225,6,0,0.15)] scroll-reveal hover:-translate-y-2"
                            style="transition-delay: <?= $delay ?>ms;">

                            <!-- اطلاعات مخفی برای موتورهای جستجو (SEO) -->
                            <meta itemprop="name" content="<?= e($p['name']) ?>" />
                            <meta itemprop="sku" content="<?= e($p['oem']) ?>" />
                            <meta itemprop="description"
                                content="خرید <?= e($p['name']) ?> اصلی با ضمانت اصالت برای <?= e($carModelName) ?>" />
                            <link itemprop="url" href="<?= $hostUrl ?>/product/<?= $safeSlug ?>" />

                            <!-- برچسب‌های شناور -->
                            <div
                                class="absolute top-3 left-3 right-3 flex justify-between items-start z-10 pointer-events-none">
                                <?php if ($p['isGenuine']): ?>
                                    <span
                                        class="bg-emerald-500/90 text-white text-[10px] font-bold px-2.5 py-1 rounded-md shadow-md backdrop-blur-sm flex items-center gap-1">
                                        <i data-lucide="shield-check" class="w-3 h-3"></i> جنیون پارت
                                    </span>
                                <?php else: ?>
                                    <span
                                        class="bg-brand-dark/80 text-gray-200 text-[10px] font-bold px-2.5 py-1 rounded-md border border-white/10 backdrop-blur-sm">OEM</span>
                                <?php endif; ?>
                                <span
                                    class="bg-brand-red text-white text-[10px] font-bold px-2.5 py-1 rounded-md shadow-[0_0_15px_rgba(225,6,0,0.5)] animate-pulse">جدید</span>
                            </div>

                            <!-- تصویر محصول -->
                            <a href="/product/<?= $safeSlug ?>"
                                class="h-56 bg-brand-dark/50 flex items-center justify-center p-6 border-b border-white/5 relative overflow-hidden block">
                                <img itemprop="image" src="<?= $img ?>" loading="lazy" alt="عکس و قیمت <?= e($p['name']) ?>"
                                    title="خرید <?= e($p['name']) ?> تویوتا"
                                    class="max-w-full max-h-full object-contain group-hover:scale-110 transition-transform duration-700 ease-out drop-shadow-xl relative z-10">

                                <!-- افکت سایه داخلی تصویر در حالت هاور -->
                                <div
                                    class="absolute inset-0 bg-gradient-to-t from-brand-dark via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                                </div>
                            </a>

                            <!-- محتوای متنی کارت -->
                            <div class="p-5 flex-1 flex flex-col justify-between space-y-4 relative z-10 bg-brand-grey">
                                <div class="space-y-2">
                                    <p class="text-[10px] text-gray-500 font-mono tracking-wider">OEM:
                                        <?= e($p['oem']) ?>
                                    </p>

                                    <a href="/product/<?= $safeSlug ?>" class="block">
                                        <h3
                                            class="font-bold text-sm text-gray-100 group-hover:text-brand-red transition-colors duration-300 line-clamp-2 leading-loose">
                                            <?= e($p['name']) ?>
                                        </h3>
                                    </a>

                                    <p class="text-[11px] text-gray-400 flex items-center gap-1.5 pt-1">
                                        <i data-lucide="car" class="w-3.5 h-3.5 text-brand-accent"></i>
                                        <?= e($carModelName) ?>
                                    </p>
                                </div>

                                <!-- بخش قیمت و سبد خرید -->
                                <div class="flex justify-between items-end pt-4 border-t border-white/5" itemprop="offers"
                                    itemscope itemtype="https://schema.org/Offer">
                                    <meta itemprop="priceCurrency" content="IRR" />
                                    <meta itemprop="price" content="<?= $p['price'] * 10 ?>" />
                                    <link itemprop="availability" href="https://schema.org/InStock" />

                                    <div class="space-y-1">
                                        <span class="block text-[10px] text-gray-500">قیمت مصرف‌کننده:</span>
                                        <div
                                            class="font-black text-sm sm:text-base text-white tracking-wide group-hover:text-brand-red transition-colors duration-300">
                                            <?= $price ?> <span class="text-[10px] font-normal text-gray-500">تومان</span>
                                        </div>
                                    </div>

                                    <button onclick="addToCart(<?= $p['id'] ?>); event.preventDefault();"
                                        class="w-11 h-11 bg-brand-dark border border-white/10 hover:border-brand-red text-gray-400 hover:text-white hover:bg-brand-red rounded-xl flex items-center justify-center transition-all duration-300 shrink-0 group/btn shadow-sm">
                                        <i data-lucide="shopping-cart"
                                            class="w-5 h-5 group-hover/btn:scale-110 group-hover/btn:-rotate-6 transition-transform duration-300"></i>
                                    </button>
                                </div>
                            </div>
                        </article>
                        <?php
                        $delay += 100; // تاخیر 100 میلی ثانیه ای برای اجرای آبشاری انیمیشن
                    endforeach;
                    ?>
                </div>
            </div>
