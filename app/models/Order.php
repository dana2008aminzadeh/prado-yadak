    ...
    public static function createOrder(array $orderData, array $items): array
    {
        ...
        try {
            ...
            $sql = "INSERT INTO orders (
                ...
                receipt_path, subtotal, discount_amount, total_amount, 
            ...
            $stmt->execute([
                ...
                ':bank_reference' => $orderData['bank_reference'],
            ...
            // درج اقلام فاکتور منطبق با ساختار order_items
            ...
            // پاک‌سازی سبد خرید کاربر از دیتابیس
            $db->prepare("DELETE FROM cart_items WHERE user_id = ?")->execute([$orderData['user_id']]);

            // ثبت استفاده از کوپن در صورت وجود
            if (!empty($orderData['applied_coupon'])) {
                Coupon::incrementUsage($orderData['applied_coupon']);
            }
